...
import { APIKeys, APIKeyProvider, APIKeyStatus, LLM_PROVIDER_INFO } from '../../../shared/types/index.js';
...
export async function getAllAPIKeys(): Promise<APIKeys> {
  ...
  try {
    ...
    return JSON.parse(content) as APIKeys;
  } catch (error) {
 ...
 */
export async function getAPIKeyByProvider(provider: string): Promise<string | null> {
 ...
 */
export async function updateAPIKey(provider: string, key: string): Promise<boolean> {
  const providerInfo = LLM_PROVIDER_INFO.find(p => p.id === provider);
  if (!providerInfo) {
    return false;
  }
