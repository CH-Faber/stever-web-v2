import { ModelInfo, ModelCapability, CustomEndpoint, LLM_PROVIDER_INFO } from '../../../shared/types/index.js';
...
/**
 * OpenAI API model response format
 */
 ...
 */
function parseOpenAIModels(response: OpenAIModelResponse, provider: string): ModelInfo[] {
 ...
 */
function getCacheKey(provider: string, endpointId?: string): string {
 ...
 */
function getPredefinedModels(provider: string): ModelInfo[] {
  const predefinedModels: Record<string, ModelInfo[]> = {
    anthropic: [
 ...
 */
export async function fetchModelsFromEndpoint(endpoint: CustomEndpoint): Promise<ModelInfo[]> {
 ...
 */
export function clearCache(provider?: string, endpointId?: string): void {
 ...
 */
export function isCached(provider: string, endpointId?: string): boolean {
